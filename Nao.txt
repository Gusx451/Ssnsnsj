-- Blue Hub V6.2 Troll Players Fling Couch + Skybox/Planking (com aba Sound + botão Parar Música já aplicado + RolePlayName/RolePlayBio com cores aplicadas)
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Lindao10/Redzlib-cor-neo/refs/heads/main/RedzHubUI_NeoBlack_COMPLETO_EDITADO.lua",true))()
local Window = Library:MakeWindow({
    Title = "Blue Hub - Brookhaven 🏡RP",
    SubTitle = "by GuzTer_666 And Anomino666",
    LoadText = "Blue Hub V6.2",
    Flags = "BlueHub_Brookhaven"
})

-- 🔊 Som ao iniciar o Hub (JÁ APLICADO)
do
...
-- Botão minimizar personalizado
Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://17573515342", BackgroundTransparency = 0 },
...
-- Agora também aplica as cores: nome = Azul, bio = Vermelho
do
    pcall(function()
        ...
        if RE and RE:FindFirstChild("1RPNam1eTex1t") then
            pcall(function()
                RE["1RPNam1eTex1t"]:FireServer("RolePlayName", "B l u e  U S E R")
            end)
            task.wait(0.15)
            pcall(function()
                RE["1RPNam1eTex1t"]:FireServer("RolePlayBio", "Scripter")
            end)
        else
            if Remotes and Remotes:FindFirstChild("RPNameText") then
                pcall(function()
                    Remotes.RPNameText:FireServer("RolePlayName", "B l u e  U S E R")
                end)
                task.wait(0.15)
                pcall(function()
                    Remotes.RPNameText:FireServer("RolePlayBio", "Scripter")
                end)
            end
        end

        -- Função que tenta aplicar cores em múltiplos remotes (strings e Color3 como fallback)
        ...
        -- Primeiro tenta enviar strings (ex: "Blue"/"Red") — muitos remotes aceitam isso
        tryApplyColors("Blue", "Red")
        task.wait(0.06)
        -- Em seguida envia Color3 como fallback (alguns remotes requerem Color3)
        tryApplyColors(Color3.fromRGB(0, 120, 255), Color3.fromRGB(255, 0, 0))
...
MainTab:AddButton({
    Name = "Bem-Vindo No Blue Hub V6.2!",
...
-- === Aba Skybox/Planking ===
----------------------------------------------------------------
local SkyTab = Window:MakeTab({Title = "SkyBox", Icon = "rbxassetid://137000362674967"})

-- [ADICIONADO: Seção de Skybox Custom]
...
-- === Aba Avatar ===
----------------------------------------------------------------
local Tab3 = Window:MakeTab({Title = "Avatar", Icon = "rbxassetid://107000362674967"})
...
TeleportTab:AddDropdown({
    ...
    Multi = false,
    Options = {"Morro","Praça","Banco","Hospital","Prefeitura","Fazenda","Mercado","Shopping","Aeroporto","Hotel","Beira-mar 1","Beira-mar 2"},
    Callback = function(value)
...
SoundTab:AddButton({
    ...
    Callback = function()
        if inputSoundId == "" then
            notify("Sound", "Insira um ID de som válido antes de tocar.", 3)
            return
        end
...
-- === Aba RGB (ADICIONADA) ===
----------------------------------------------------------------
local Tab13 = Window:MakeTab({Title = "RGB", Icon = "rbxassetid://107000362674967"})

Tab13:AddSection({"Funções RGB"})
...
-- Velocidade do RGB (1..5 interpretado pelo usuário)
local rgbSpeed = 3

-- Função para criar cor rainbow suave com HSV
...
-- Helper para disparar remotes com segurança (procura RE depois Remotes)
...
----------------------------------------------------------------
print("Dragon Hub V5.8 - Script carregado com RolePlayName/RolePlayBio e aba RGB aplicados.")